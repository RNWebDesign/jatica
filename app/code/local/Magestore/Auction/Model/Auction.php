...
class Magestore_Auction_Model_Auction extends Mage_Core_Model_Abstract
{
    ...
    {
        //return Mage::helper('core')->currency($this->getPrice());
        return  Mage::helper('core')->currency(($this->getPrice() * 1.17) * 1.21) ;
    }
	...
	// Lay 20 bidder cuoi cung
	public function getTwentyBidderLastest($productAuctionId, $limitTime, $limitDate) {
		$collection = $this->getCollection()
			->addFieldToFilter('created_date', array('gteq'=>$limitDate))
			->addFieldToFilter('created_time', array('gteq'=>$limitTime))
			->addFieldToFilter('productauction_id', $productAuctionId)
			;
		// $collection->printlogquery(true); die('xxxx');
		return $collection;
		
	}
	
	public function sendNoticToAllBider($customer_id, $production_id,$lastHighestBidder){
        ...
        if(count($collection))
		{
			foreach ($collection as $item) {
                ...
                if ($item->getCustomerId() != $customer_id && $item->getCustomerId() == $lastHighestBidder){
