<?php
class Magestore_Auction_Block_Customer_Bidlist_if_won extends Mage_Core_Block_Template
{
	public function _prepareLayout()
    {
		...
		if($type_biddername == '2')//user create bidder name
		{
			...
			{
				$this->setTemplate('auction/customer/customerbid_check_if_won.phtml');
			}else{
	...
	public function getNavHtml()
	{
		...
		if($last_page >1)
		{
			...
			{
				if($i != $curr_page)
					$html .= '<a href="'. $this->getUrl('auction/index/customerbid_check_if_won',array('page'=>$i)) .'" >'. $i .'</a>';
				else
