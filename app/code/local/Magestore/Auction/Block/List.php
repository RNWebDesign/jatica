...
class Magestore_Auction_Block_List extends Mage_Catalog_Block_Product_List
{
    ...
    protected function _getProductCollection()
    {
        if (is_null($this->_productCollection)) {
			...
			$categ = $this->helper('core/url')->getCurrentUrl();
			...
			if (!$cat & !(Mage::app()->getRequest()->getRequestString() == "/") & !(Mage::app()->getRequest()->getRequestString() == "/auction/index/index/") & !(Mage::app()->getRequest()->getRequestString() == "/auction/index/")) {
			$cat = Mage::registry('current_category')->getId();
				}
			
            if (!$cat) {
                Mage::log('categorie empty ');
