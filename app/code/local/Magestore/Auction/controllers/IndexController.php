...
class Magestore_Auction_IndexController extends Mage_Core_Controller_Front_Action
{
	public function indexAction()
    {
		...
		$this->getLayout()
				->getBlock('head')
				->setTitle(Mage::helper('core')->__('Jatica - Veilingen en Stockverkoop'));
	...
	public function checkbiddernameAction()
	{
		...
		if(count($collection)){
			...
			$html .= '<div class="error-msg"><p>'.Mage::helper('core')->__('This bidder name already exists').'</p></div>';
		}else{
	...
	public function watchlistAction()
	{
		...
		$this->getLayout()
				->getBlock('head')
				->setTitle(Mage::helper('core')->__('My Watched Auctions'));
	...
	public function bidAction()
	{
		...
		if($bidType){
			...
			try{
				...
				$auctionbid->sendNoticToAllBider($auctionbid->getCustomerId(), $auctionbid->getProductauctionId(),$lastAuctionBid->getCustomerId());
			...
			if($check_autobid == 1){
				...
				try{
					...
					$result .= $notice->getNoticeSuccess($_helper->__('Proxybod succesvol'));
					$this->getResponse()->setBody($result);
				}catch(Exception $e){
					$result .=  $notice->getNoticeError($e->getMessage());
					$this->getResponse()->setBody($result);		
				...
				$result .= $notice->getNoticeError($_helper->__('Je hebt een proxybod geplaatst voor dit item'));
				$this->getResponse()->setBody($result);						
				return;
			}
		}									
	...
	public function cancelBidAction()
	{
		...
		try{
			...
			Mage::getSingleton('core/session')->addSuccess(Mage::helper('auction')->__('Bod geannuleerd!'));
			$this->_redirect('auction/index/customerbid');
		} catch(Exception $e){
			Mage::getSingleton('core/session')->addSuccess(Mage::helper('auction')->__('Annulatie mislukt!'));
			$this->_redirect('auction/index/customerbid');
		}
	}	
